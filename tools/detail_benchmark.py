...
def summarize(paths: list[Path]) -> dict[str, Any]:
        ...
        with path.open("r", encoding="utf-8") as stream:
            for line in stream:
                ...
                if (
                    ...
                    categories[(str(path), generation)] = str(details["category"])
    ...
    for transaction_key, stages in transactions.items():
        ...
        image = (
            ...
            else stages.get("image_presented", stages.get("presented"))
        )
    ...
    return {
        ...
        "categories": {
            category: {
                name: _metric(samples)
                for name, samples in sorted(category_values.items())
            }
...
def validate_summary(summary: dict[str, Any], *, minimum_samples: int = 30) -> dict[str, Any]:
    ...
    add_check(
        ...
        route_p95,
        32.0,
        passed=isinstance(route_p95, (int, float)) and route_p95 <= 32.0,
    )
    ...
    add_check(
        ...
        gui_p95,
        24.0,
        passed=isinstance(gui_p95, (int, float)) and gui_p95 <= 24.0,
    )
    ...
    for category, metrics in sorted(summary.get("categories", {}).items()):
        ...
        limit = 80.0 if "hot" in lowered else 300.0 if any(
            marker in lowered for marker in ("raw", "heavy", "crop")
        ) else 150.0
        checks.append(
            {
