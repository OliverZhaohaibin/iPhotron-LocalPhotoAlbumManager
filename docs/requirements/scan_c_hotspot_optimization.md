...
> **状态：未来优化输入；性能数据为历史 profiling 快照。** 本文中的调用链、
> 固定耗时占比和 native rewrite 优先级不是当前性能合同。采取优化行动前，必须重新测量当前
> `ScanLibraryUseCase` / `LibrarySession` 扫描链路；只有新的目标平台 profile
> 仍证明同一热点成立时，才可采用本文的 C/ctypes 建议。

---

## 1. 扫描流程概览

iPhotron 的扫描流程在本文采样时分为以下几个阶段：
