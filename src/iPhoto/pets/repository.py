...
class PetRepository:
    ...
    def complete_runtime_state_sync(self, operation_id: str) -> dict[str, object] | None:
        ...
        if str(payload.get("operation_kind") or "") == "pet_delete_detection":
            ...
            if face_state_path.exists() and detection_id:
                ...
                clear_assignment = getattr(
                    face_state,
                    "clear_annotation_identity_assignment",
                    None,
                )
                if callable(clear_assignment):
                    clear_assignment("pet", detection_id)
    ...
    def get_asset_ids_by_pet(self, pet_id: str) -> list[str]:
        ...
        if face_state_path.exists():
            ...
            effective_query = getattr(
                face_repository,
                "get_asset_ids_by_pets_effective",
                None,
            )
            if callable(effective_query):
                return effective_query((pet_id,)).get(pet_id, [])
        self.initialize()
    ...
    def list_asset_pet_annotations(self, asset_id: str) -> list[AssetPetAnnotation]:
        ...
        if face_state_path.exists():
            face_state = FaceStateRepository(face_state_path)
            load_assignments = getattr(
                face_state,
                "get_annotation_identity_assignments",
                None,
            )
            if callable(load_assignments):
