...
def apply_adjustments(
    ...
    _sh_amount = float(adjustments.get("Sharpen_Amount", 0.0))
    sharpen_enabled = _sh_flag and abs(_sh_amount) > 1e-6
    ...
    if transformed is not None:
        ...
        if sharpen_enabled:
            transformed = _apply_sharpen_to_qimage(transformed, _sh_amount)
    ...
    if sharpen_enabled:
        result = _apply_sharpen_to_qimage(result, _sh_amount)
...
def _apply_sharpen_to_qimage(image: QImage, amount: float) -> QImage:
    """Apply unsharp-mask sharpening to a QImage."""
    if abs(amount) < 1e-6:
        return image
    ...
    arr = np.frombuffer(ptr, dtype=np.uint8).reshape((height, width, 4)).copy()
    arr = apply_sharpen_cpu(arr, amount)
