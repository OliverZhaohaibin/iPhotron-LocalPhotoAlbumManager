...
- 第二阶段当前状态：**核心链路已落地，故障恢复、慢存储策略和 packaged 验证待闭环**。
...
当前 schema 迁移已移出 GUI 线程，但 helper 被超时终止、系统断电或进程崩溃时，还缺少完整的“检测上次迁移中断并恢复”协议。
...
- SQLite `busy/locked`、损坏数据库、未来 schema、只读库和只读配置目录。
- 迁移中断及下次启动恢复。
- helper 超时、崩溃、输出截断、错误 JSON 和旧结果迟到。
- 保存路径是断开的符号链接、权限拒绝、被拔出的移动盘。
- 探测期间切库和关闭窗口，确认不会提交旧 session、不会启动扫描。
...
1. 先补 probe/SQLite 故障协议和中断恢复测试；这是第二阶段真正的稳定性门禁。
2. 再实现 GUI job 时间片观测，不急于拆更多对象，先得到超过 100 ms 的准确调用点。
