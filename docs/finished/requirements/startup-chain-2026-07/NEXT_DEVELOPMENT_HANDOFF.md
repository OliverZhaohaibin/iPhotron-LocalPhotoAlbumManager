...
> **状态：2026-07 历史 handoff，已被取代。** 本文中的 Recognition 首用、
> `DesktopCoordinatorRuntime` 和平台差异化 surface 指令不是当前实施说明。
> 当前行为见 [`docs/architecture.md`](../../../architecture.md)，当前验收见
> [`STARTUP_MANUAL_VALIDATION_MATRIX.md`](../../../requirements/STARTUP_MANUAL_VALIDATION_MATRIX.md)。
...
> `PETS_REVIEW_REMEDIATION_LEDGER.md` 的 R2-01～R2-13，再执行当时的人工矩阵；
> 当前 stacked PR 因 workflow 仅监听 `main` 而没有
...
定向验收覆盖 terminal 唯一性、取消后的迟到 import、Recognition 首用预热、构建清单和 A/B 拒绝路径。当前平台性能协议见
...
本文已因后续跨平台 QRhi lifecycle 重构而归档；尚未完成的正式人工矩阵和匹配
build manifest 的 packaged 证据继续由当前 active validation 文档跟踪。
