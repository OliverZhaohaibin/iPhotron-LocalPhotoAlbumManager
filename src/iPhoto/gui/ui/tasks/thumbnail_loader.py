...
class ThumbnailJob(QRunnable):
    ...
    def _apply_geometry_and_crop(self, image: QImage, adjustments: Dict[str, float]) -> Optional[QImage]:
        ...
        """
        from ..widgets import perspective_math
        from ..widgets.gl_image_viewer import geometry
        ...
        log_cx, log_cy, log_w, log_h = geometry.texture_crop_to_logical(tex_crop, rotate_steps)
        ...
        matrix_inv = perspective_math.build_perspective_matrix(
            vertical=p_vert,
            horizontal=p_horz,
        ...
        # Desired: T(-0.5) * S(2.0) => 2(p - 0.5) = 2p - 1.
        # Construction: S(2.0).translate(-1.0) (since translate applies after scale in builder order: p*S*T)
        t_to_ndc = QTransform().scale(2.0, 2.0).translate(-1.0, -1.0)
        
        # 4. Denormalize from [-1, 1] to [0, 1]
        # Desired: T(1.0) * S(0.5) => 0.5(p + 1) = 0.5p + 0.5.
        # Construction: S(0.5).translate(0.5)
