...
本轮新增完成的是收口审查后的 residual fix：恢复完整非 GUI/UI 测试为绿色，
并补强 GUI/library runtime 的 session fallback guardrail。上一轮
`28-performance-baseline.md` 中 Phase 6 的 scan、gallery pagination、
thumbnail cache baseline 继续有效，历史不可运行 benchmark 入口已清理。

这一步有四个关键落点：

- `prune_index_scope()` 对 Recently Deleted exclude 改为保留 `.Trash` index
  residual，不再要求 trash 文件仍存在后才保留 restore metadata。
- GUI/library runtime 中直接 `LibraryScanService(...)` /
  `LibraryAssetQueryService(...)` / lifecycle/operation service constructor
  fallback 已迁到显式 `create_compat_*` factory；主路径继续优先使用 session-bound
  surface。
- `tools/check_layer_boundaries.py` 新增 direct session-service fallback
  constructor guardrail，并有 architecture regression 覆盖。
- 完整非 GUI/UI 测试恢复通过，Recently Deleted 在 library/trash rescan 后仍可
  restore 的端到端回归单独验证通过。

## 2. 本轮完成：Residual 收口与 Guardrail

- 修复完整非 GUI/UI 测试中暴露的两个 residual：
  `prune_index_scope()` 不能在 library rescan 时删除 `.Trash` restore metadata；
  旧测试不再 patch 已退役的 lifecycle constructor。
- GUI/library runtime 的 session service constructor fallback 已统一迁到
  `bootstrap.service_factories.create_compat_*`，主路径仍优先使用 active
  `LibrarySession` 绑定到 `LibraryManager` 的 service surface。
- `tools/check_layer_boundaries.py` 新增 constructor fallback guardrail，
  `tests/architecture/test_layer_boundaries.py` 已覆盖该规则。
- 上一轮 Performance Baseline 仍有效：`tools/benchmarks/benchmark_refactor.py`
  已删除，`tools/testbase/` 继续作为本地可选输入，scan/gallery/thumbnail
  小数据 baseline 继续在 `tests/performance` 中执行。
...
- Session fallback guardrail：
  GUI/library runtime 不再直接构造 bootstrap session service fallback；兼容路径统一
  通过显式 factory，使新 guardrail 可以阻止 constructor fallback 再次散落。
...
- Phase 6：部分完成。
  ...
  baseline 已补齐；`tests/infrastructure -q` 与 CI 等价非 GUI/UI 测试当前通过。
  性能 baseline 是 regression sanity check，不代表跨机器绝对性能认证。
...
- `tools/testbase/` 是本地真实素材集，已被 `.gitignore` 忽略；CI baseline 使用
  合成数据，不依赖该目录。
- GUI/library runtime 的 standalone/test fallback 现在统一通过
  `bootstrap.service_factories.create_compat_*` 显式创建；active GUI 主路径仍应优先
  使用 `LibrarySession` 绑定到 `LibraryManager` 的 service surface。

## 6. 最新验证

本轮在项目 `.venv` 下执行：

- `.venv/bin/python -m pytest tests/application/test_library_asset_lifecycle_service.py tests/test_library_manager.py -q`
- `.venv/bin/python -m pytest tests/test_index_sync_service.py tests/application/test_library_asset_lifecycle_service.py -q`
- `.venv/bin/python -m pytest tests/application/test_temp_library_end_to_end.py::test_library_rescan_does_not_break_recently_deleted_restore -q`
- `.venv/bin/python -m pytest tests/services tests/library tests/ui/tasks/test_import_worker.py tests/gui/coordinators/test_main_coordinator_asset_runtime_boundary.py -q`
- `.venv/bin/python -m pytest tests/architecture tests/infrastructure tests/performance -q`
- `.venv/bin/python -m pytest tests/ --ignore=tests/gui --ignore=tests/ui --tb=short -q`
- `python3 tools/check_architecture.py`

结果：

- 本轮最小红测回归通过（`23 passed`）。
- index/lifecycle prune 回归通过（`15 passed`）。
- restore-after-rescan 端到端回归通过（`1 passed`）。
- GUI/library/task focused regressions 通过（`64 passed`）。
- architecture/infrastructure/performance 基线通过（`95 passed, 1 skipped`）。
- CI 等价非 GUI/UI 集通过（`1426 passed, 8 skipped`）。
...
- 仍有既有的 Qt `QMouseEvent` deprecation warnings。
- 本轮新增验证意图：rescan 后 Recently Deleted restore 不丢 index metadata；
  GUI/library runtime constructor fallback 有架构门禁。
