...
Linux 地图运行时说明：
- 当共享库可用时，iPhotron 在 Linux 上既可使用 helper-backed OBF renderer，也可使用原生 OsmAnd widget。
- 如果仓库旁边存在 `PySide6-OsmAnd-SDK/` 工作区，Linux 开发环境会优先使用其中 `tools/osmand_render_helper_native/dist-linux/` 的 widget 构建产物。
...
- macOS 当前仍回退到现有的 Python / legacy 地图路径，原生运行时仍在完善中。
...
该仓库维护 vendored OsmAnd 源码、Windows/Linux 构建脚本、原生 Qt Widget bridge
...
“Assign a Location” 搜索、选择并确认地点。如果 maps extension 缺失，面板会提供下载入口，
而不是静默失效。
...
| [Development](../development.md) | 开发环境、依赖、调试，以及面向 Windows / Linux 的 maps extension 完整构建流程 |
| [Executable Build](../misc/BUILD_EXE.md) | Nuitka 打包、AOT、maps extension 同步与 Windows / Linux 运行时说明 |
