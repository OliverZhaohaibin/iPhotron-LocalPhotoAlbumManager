...
> **状态：包含当前未关闭技术债及已关闭项的历史证据。** 本文已从历史 startup
> requirements 目录迁移到 `docs/technical-debt/`。TD-890-02～05 仍为 active；
> TD-890-01 已由 PR #903 关闭并保留在本文的 closed section。
...
本台账用于后续独立实施，不得把 Active 技术债务总览中的事项误报为 PR #902 已完成。实现时应先取得
...
## Closed Items / Historical Closure

| ID | 原优先级 | 关闭结论 | 状态 |
| --- | --- | --- | --- |
| TD-890-01 | P1 | PR #903 完成 SQLite index、trusted mmap hit、分块写入、维护水位和跨进程 cache namespace ownership；对应自动化与三平台 cache artifacts 已通过 | `closed` |

### TD-890-01：已关闭 — 磁盘 surface cache 的线性成本
...
1. TD-890-02 的观测与基线：建立 I/O、延迟、RSS、owner/lease 数据。
2. TD-890-03：定位 Windows 50k×384 热点，使当前 CI 证据稳定。
3. TD-890-02：按 CPU/session → RAW → GPU 顺序迁移统一所有权。
4. TD-890-04：在向 `edit-base`/`main` promotion 前闭环触发和 required checks。
5. TD-890-05：作为后续所有大型改造的持续交付规则执行。

## 关闭规则

- Active 项只能更新为 `in_progress`、`automated_pass`、`manual_pending` 或附证据的
  `not_applicable`；只有验收条件和所需平台证据闭环后才能移入 closed section 并标记
  `closed`，不得仅凭“代码已提交”关闭。
