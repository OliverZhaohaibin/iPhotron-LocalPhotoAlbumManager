...
class VideoArea(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        # blend with the background, producing a washed-out / grey picture.
        # Letterbox areas therefore appear black — the industry standard for
        # video players — while the widget-level backgrounds track the theme's
        # surface colour for surrounding chrome consistency.
    ...
    def set_surface_color_override(self, colour: str | None) -> None:
        ...
        Called by the theme controller when the application switches between
        light and dark mode so the letterboxed areas around the video frame
        always match the surrounding chrome.
        """
    ...
    def _apply_surface_color(self, colour: str) -> None:
        """Apply *colour* to widget and viewport backgrounds.

        Only the widget-level backgrounds are updated so that any exposed
        chrome outside the QGraphicsView matches the current theme.  The
        QGraphicsScene background is **always** kept black for correct
        HDR / HEVC compositing — see ``__init__`` for details.
        """
        ...
        # Scene background intentionally NOT changed — must stay black.
    ...
    def _on_media_status_changed(self, status: QMediaPlayer.MediaStatus) -> None:
        if status == QMediaPlayer.MediaStatus.EndOfMedia:
            ...
            if position + 200 < duration:
                return
            # Freeze on the last frame instead of transitioning to a black
            # screen.  Seeking first sets the decode position; pausing then
            # keeps the decoder output visible.
            self._player.setPosition(max(0, duration - 1))
            self._player.pause()
            self.show_controls()
            self.playbackFinished.emit()
